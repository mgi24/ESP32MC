...
uint8_t makeBlockChange (short x, uint8_t y, short z, uint8_t block) {
  ...
  ChunkAnchor anchor = (ChunkAnchor){
    .x = x / CHUNK_SIZE,
    .z = z / CHUNK_SIZE,
    .hash = 0,
    .biome = 0
...
void handleServerTick (int64_t time_since_last_tick) {
  ...
  for (int i = 0; i < MAX_MOBS; i ++) {
    ...
    if ( // Validate movement on X axis
      (new_x != old_x && !isPassableBlock(getBlockAt(new_x, new_y + 1, old_z))) ||
      (
    ...
    if ( // Validate movement on Z axis
      (new_z != old_z && !isPassableBlock(getBlockAt(old_x, new_y + 1, new_z))) ||
      (
