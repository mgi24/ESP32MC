...
static void print_memory_stats(const char *tag) {
  ...
  size_t used           = total_internal - free_heap;
  printf("[MEM]%s Used: %u / %u  (%.1f%%)  Free:%u  MinFree:%u  LargestBlk:%u\n",
         tag ? tag : "",
         (unsigned)used,
         (unsigned)total_internal,
         (double)used * 100.0 / (double)total_internal,
         (unsigned)free_heap,
...
int main () {
  ...
  printf("Server listening on port %d...\n", PORT);
    ...
    int64_t last_mem_log = get_program_time();
  #endif
  ...
  while (true) {
    ...
    if (now_us - last_mem_log > 1000000) {
      print_memory_stats("");
      last_mem_log = now_us;
    }
